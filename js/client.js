...
(function() {
    ...
    var chatScrollBar = {
        ...
        init: function() {
            ...
            if (!this.chtblock || !this.vsblBtn || !this.track || !this.thumb)
                return false;
        ...
        _updateScrollBar: function() {
            ...
            if (percent * t2 >= 0 && percent * t2 <= t1)
                this.thumb.style.bottom = percent * t2 + 'px';
    ...
    function filterBigString(ctx, name, maxW, maxH) {
        while (true) {
            ...
            const h = Math.abs(m.actualBoundingBoxDescent) +
    ...
    function drawStats() {
        ...
        var rows = [stats.info.name + ' (' + stats.info.mode + ')', stats.info.playersTotal + ' / ' + stats.info.playersLimit + ' players', stats.info.playersAlive + ' playing', stats.info.playersSpect + ' spectating', (stats.info.update * 2.5).toFixed(1) +
            '% load @ ' +
    ...
    function drawGrid() {
        ...
        var step = 50,
            i, cW = mainCanvas.width / camera.scale,
    ...
    function drawGame() {
        ...
        if (settings.showChat && chat.visible) {
            mainCtx.drawImage(chatScrollBar.wrapTheChat(chat.canvas), (10 + chatScrollBar.chatLeftOffset) / camera.viewportScale, (mainCanvas.height - 54) / camera.viewportScale -
                chatScrollBar.wrapperHeight);
            mainCtx.globalAlpha = 1;
        }
        drawMinimap();
        drawPosition();
    ...
    Cell.prototype = {
        ...
        setSkin: function(value) {
            if (value) {
                ...
                const skinPath = value.replace(/^1\%/, '/free/').replace(/^2\%/, '/level/').replace(/^3\%/, '/premium/') + '.png';
    ...
    window.setserver = function(url) {
        ...
        serverURL = serverURL.indexOf('localhost') !== -1 ? serverURL.replace('localhost/server', 'localhost:3003/server') : serverURL;
...
function signOut() {
    ...
    auth2.signOut().then(function() {
        ...
        if (skinSelectIcon) {
            skinSelectIcon.style.backgroundImage = `url("${serverProtocol}://${serverURL.replace('server','assets')}/images/checkerboard.png")`;
        }
        if (skinSelectIconText) {
...
function drawGalleryHandler(skinsP, tab, newSkin = null, searchQuery = searchOut) {
    if (searchQuery === searchOut)
        document.querySelector('#skin-search').value = '';
    const server = `${serverProtocol}://${serverURL}`;
    const skins = {
        free: '',
        level: '',
...
function getTopUsers() {
    ...
    fetch(server + '/topusers', {
        ...
        for (var i = 0; i < data.body.topUsers.length; i++) {
            document.getElementsByClassName('top-users__list')[0].innerHTML += '<tr><td>' +
                (i + 1) +
                '</td><td>' +
...
const getServersStats = async () => {
    ...
    await Promise.all(servers.map(async (item, index) => {
        ...
        try {
            ...
            document.getElementById('option_' + index).innerHTML = servers_names[index] +
                ' ' +
                data.body.serverstats.players_current +
                '/' +
                data.body.serverstats.players_max;
...
function Share(type) {
    if (type == 'fb') {
        window.open('http://www.facebook.com/sharer.php?u=https://sigmally.com&quote=Play+to+Sigmally!%20Highest%20Mass%20' +
            ...
            '%20Food%20Eaten%20' +
            document.getElementById('food_eaten').innerHTML, 'pagename', 'resizable');
    }
    if (type == 'tw') {
        window.open('https://twitter.com/intent/tweet?text=Play%20To%20sigmally.com%20Highest%20Mass%20' +
            document.getElementById('highest_mass').innerHTML +
            '%20Top%20Position%20' +
            document.getElementById('top_leaderboard_position').innerHTML +
            '%20Time%20Alive%20' +
